...
public class TEACHERAssessmentTest {
	...
	public void globalTest() {
		...
		assertEquals(Room.Type.SINGLE.toString(), roomBookingData.getRoomType().toString());
		...
		for (int i = 0; i < 5; i++) { // FIXME (ERROR): should have ConfirmedState.MAX_BANK_EXCEPTIONS
			adventure.process();
		}
		Assert.assertEquals(State.UNDO, adventure.getState());
		...
		adventure.setPaymentConfirmation("FAKE CONFIRMATION");
		for (int i = 0; i < 5 - 1; i++) { // FIXME (ERROR): should use ConfirmedState.MAX_BANK_EXCEPTIONS
			adventure.process();
		}
		...
		assertEquals(offerAvailability, this.offerArrivalDeparture.getNumberOfBookings()); // FIXME (ERROR): getNumberOfBookings() should be public
		...
		// test getRoomBookingData for cancellation  // FIXME (ERROR): pt.ulisboa.tecnico.softeng.hotel.exception.HotelException
//		roomBookingData = HotelInterface.getRoomBookingData(adventure.getRoomCancellation());
//		assertEquals(Room.Type.SINGLE.toString(), roomBookingData.getRoomType().toString());
//		assertNotNull(roomBookingData.getCancellation());
		...
		for (int i = 0; i < BulkRoomBooking.MAX_HOTEL_EXCEPTIONS + 1; i++) { // FIXME (ERROR): MAX_HOTEL_EXCEPTIONS should be public
		...
		// test Ok bulk
		String reference = bulkRoomBooking.getReference(Room.Type.DOUBLE.toString());
		assertFalse(bulkRoomBooking.getReferences().contains(reference));
		assertEquals(this.roomCounter - 2, bulkRoomBooking.getReferences().size());
		assertEquals(Room.Type.DOUBLE.toString(), Hotel.getRoomBookingData(reference).getRoomType());
		...
		reference = room.reserve(Type.DOUBLE, arrival, departure).getReference();
